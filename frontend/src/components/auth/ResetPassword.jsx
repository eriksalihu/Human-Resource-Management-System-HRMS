 ...
 * @description Reset-password form — minimal, light-only
 * @author Dev B (original), Dev A (light-only redesign)
 */
...
const ResetPassword = () => {
  ...
  if (!token) {
    return (
      <div className="space-y-5 text-center">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900">Reset link missing</h2>
    ...
    return (
      <div className="space-y-5 text-center" role="status" aria-live="polite">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100">
          <svg className="h-6 w-6 text-emerald-600" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden="true">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      {error && (
        <div role="alert" className="rounded-md bg-rose-50 border border-rose-200 px-3 py-2 text-sm text-rose-700">
          {error}
        </div>
      )}
      ...
      <div>
        ...
        <input
          ...
          className="block w-full rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm bg-white text-gray-900 placeholder-gray-400 border-gray-300"
        />
        <PasswordStrengthMeter id="reset-password-strength" password={password} className="mt-2" />
      </div>
      ...
      <div>
        ...
        {confirm && confirm !== password && (
          <p className="mt-1 text-xs text-rose-600">Passwords don't match yet.</p>
        )}
      </div>
